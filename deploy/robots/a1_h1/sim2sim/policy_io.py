...
DEFAULT_POLICY = (
    LGY_ROOT
    / "Pingpong_TTRL/logs/a1_tt_v9/2026-07-07_02-50-16/exported/policy.onnx"
)
...
KP = np.array([200.0, 200.0, 200.0, 90.0, 90.0, 90.0, 90.0], dtype=np.float64)
KD = np.array([3.5, 3.5, 3.5, 0.5, 0.5, 0.5, 0.5], dtype=np.float64)
EFFORT = np.array([28.0, 28.0, 28.0, 8.0, 8.0, 8.0, 8.0], dtype=np.float64)
VEL_LIMIT = np.array([8.0, 8.0, 8.0, 20.0, 20.0, 20.0, 20.0], dtype=np.float64)
# h1_pingpong/src/armcontrol/src/cartesian_servo_node.cpp config:
# r1-r4 DM4340, r5-r7 DM4310. These are DAMIAO command/feedback ranges,
# not a calibrated motor dynamics model.
DAMIAO_EFFORT = np.array([28.0, 28.0, 28.0, 28.0, 10.0, 10.0, 10.0], dtype=np.float64)
DAMIAO_DQ_LIMIT = np.array([8.0, 8.0, 8.0, 8.0, 30.0, 30.0, 30.0], dtype=np.float64)
...
class A1PolicyIO:
    ...
    def update_action(self, raw_action: np.ndarray) -> np.ndarray:
        ...
        self.q_des = np.clip(q_des, self.q_min, self.q_max)
    ...
    def estimate_mit_tau(self, effort_limit: np.ndarray | None = None) -> np.ndarray:
        ...
        tau = KP * (self.q_des - q) + KD * (0.0 - dq) + self.data.qfrc_bias[self.dof_addr]
    ...
    def apply_position_servo(
        ...
        if dt > 0.0:
            ...
            desired_dq = (self.q_des - self.servo_q) / max(float(tracking_tau), 1e-6)
